...
def admin_instance_class(model, base=DetailView):
    if hasattr(model, 'custom_admin_template'):
        ...
        tmpl = loader.select_template([
            'suit/streamblocks/admin/%s.html' % model.__name__.lower(),
...
def delete_instance(request, app_label, model_name, pk):
    t = ContentType.objects.get(app_label=app_label, model=model_name)
