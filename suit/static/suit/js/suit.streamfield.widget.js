(function (w, $) {
    ...
    function onReady() {
        ...
        function initStreamApp(app_node) {
            ...
            // prepare stream data
            data.stream = _.map(data.stream, (item) => {
                let obj = data.model_info[item.model_name];
                item.admin_url = obj.admin_url;
                item.app = obj.app;
                return item
            })
            ...
            var app = new Vue({
                ...
                methods: {
                    ...
                    app_name_lower: function (block) {
                        return block.app.toLowerCase()
                    },
                    ...
                    block_admin_url: function (block) {
                        return block.admin_url;
                    },
                    instance_admin_render_url: function (block, instance_id) {
                        return '/suit/admin-instance/' + this.model_name_lower(block) + '/' + instance_id;
                    },
                    abstract_block_render_url: function (block) {
                        return '/suit/abstract-block/' + this.model_name_lower(block) + '/';
                    },
                    ...
                    get_add_model_link: function (block) {
                        return `${this.block_admin_url(block)}add/?_popup=1&block_id=${block.unique_id}&app_id=${app_node.id}`;
                    },
                    ...
                    deleteAction: function (block, id, idx) {
                        return ax.delete(`/suit/admin-instance/${app.app_name_lower(block)}/${app.model_name_lower(block)}/${id}/delete`)
                    },
                    addNewBlock: function (block, model_name) {
                        ...
                        new_block = {
                            ...
                            options: options,
                            admin_url: this.model_info[model_name].admin_url
                        };
                        if (!block.abstract) {
                ...
                computed: {
                    textarea: function () {
                        ...
                        return JSON.stringify(_.stream.map(function (i) {
