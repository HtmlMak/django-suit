        ...
        for m in self.from_json():
            ...
            if model_str in self.model_list_names:
                ...
                ctx = dict(
                    ...
                    as_list=as_list,
                    admin_url=reverse('admin:%s_%s_changelist' % (model_class._meta.app_label, model_class._meta.model_name)),
                    options=m['options']
                )
...
def _get_render_data(model_class, model_str, content, ctx):
    ...
    try:
        ...
        t = loader.get_template('suit/streamfield/default_block_tmpl.html')
    return t.render(ctx)
...
def _get_render_admin_data(model_class, model_str, content, ctx):
    t = loader.select_template([
        'suit/streamblocks/admin/%s.html' % model_str.lower(),
        'suit/streamfield/admin/change_form_render_template.html'
    ])
